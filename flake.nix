{
  description = "Timeline plugin: notification — server binary + web bundle.";
  ...
  outputs = { self, timeline, flake-utils }:
    flake-utils.lib.eachDefaultSystem (system: {
      packages.default = timeline.lib.buildPlugin {
        inherit system;
        name = "timeline_plugin_notification";
        src = ./.;
      };
      packages.timeline_plugin_notification = self.packages.${system}.default;
    });
}
